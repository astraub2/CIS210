...
Input file is a CSV file containing earthquake data(coordiantes,
magnitudes, depths)in the Oregon and Washington
regions. Outputs are analysis of the depths, magnitudes, and clusters of
this data, as well as plots of the depths, magnitudes and clusters using
turtle graphics.
...
def plot_clusters(eq_clusters, eq_dict):
    ...
    """
    counter=0
    color=["red", "blue", 'yellow', "pink", "purple", "orange"]
    while counter<6:
        global eq_turtle
        clusters=eq_clusters[counter]
        for i in clusters:
            lon=eq_dict[i][0]
            lat=eq_dict[i][1]
...
def plot_magnitudes(eq_dict):
    ...
    """
    global eq_turtle
    color=["violet", "blue", "cyan", "purple", "yellow", "orange", "red","black", "white", "grey", "green"]
    size=[8,12,16,20,24,28,32,36,40,44]
    for i in eq_dict:
        mag_size=size[math.floor(eq_dict[i][2])]
        mag_color=color[math.floor(eq_dict[i][2])]
        lon=eq_dict[i][0]
        lat=eq_dict[i][1]
        x, y= xy_calculate(lon, lat)
        turtle.penup()
        turtle.goto(x, y)
        turtle.dot(mag_size, mag_color)
...
def plot_depths(eq_dict):
    ...
    """
    global eq_turtle
    color=["violet", "cyan", "blue", "yellow", "orange", "red", "red", "red", "red", "red", "red"]
    size=[10,15,20,25,30,35,35,35,35,35,35,35]
    for i in eq_dict:
        depth_size=size[math.floor((eq_dict[i][3])/5)]
        depth_color=color[math.floor((eq_dict[i][3])/5)]
        lon=eq_dict[i][0]
        lat=eq_dict[i][1]
        x, y= xy_calculate(lon, lat)
        turtle.penup()
        turtle.goto(x, y)
        turtle.dot(depth_size, depth_color)
def analyze_depths(eq_dict):
    ...
    mean= "{0:.1f}".format(data_mean(depths))
    median= "{0:.1f}".format(data_median(depths))
    m1,var=data_mean_variance(depths)
    standard= "{0:.2f}".format(math.sqrt(var))
    print("Analysis of depth data")
    print("   Mean depth= "+str(mean)+" miles")
    print("   Median depth= "+str(median)+" miles")
    print("   Strandard deviation= "+str(standard)+" miles")
    table=data_freq_table(depths)
    #print(table)
...
def analyze_magnitudes(eq_dict):
    ...
    mag=[]
    for i in eq_dict:
        mag.append(eq_dict[i][2])
    mean="{0:.1f}".format(data_mean(mag))
    median="{0:.1f}".format(data_median(mag))
    m1,var=data_mean_variance(mag)
    standard= "{0:.2f}".format(math.sqrt(var))
    
    print("Analysis of magnitude data")
    print("   Mean magnitude= "+str(mean))
    print("   Median magnitude= "+str(median))
    print("   Strandard deviation= "+str(standard))
    table=data_freq_table(mag)
    #print(table)
...
def analyze_clusters(eq_clusters, eq_dict):
    ...
    counter=0
    while counter<6:
        cluster=eq_clusters[counter]
        print("Analysis of cluster "+str(counter))
        mag=[]
        for i in cluster:
            mag.append(eq_dict[i][2])
