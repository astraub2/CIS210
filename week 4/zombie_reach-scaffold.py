...
def zombieSearch(map,zombie_x,zombie_y,target_x,target_y):
    ...
    while map:
        ...
        location=readLoc(map,x,y)
        newlocation=setLoc(map,x,y,$)
        if (zombie_x==target_x and zombie_y==target_y):
            return True
        else:
            
            newlocation=setLoc(map,up,y,$)
            newlocation=setLoc(map,down,y,$)
            newlocation=setLoc(map,x,right,$)
            newlocation=setLoc(map,x,left,$)
