...
class Circle(Shape): 
    ...
    def area(self):
        ...
        Returns the area of the shape
        """
        return self.area=self.radius^^2(math.pi)
    ...
    def circumference(self):
        ...
        Returns the circumference of the shape
        """
        return self.circumference=2*pi(self.radius)
...
# Math on shapes, build new shapes by combining the area of two different shapes
    ...
    def __sub__(self, other):
        """
        Subtracts other shape from this shape (lhs - rhs).  If the resulting
        area would be zero or less, than
        None is return
        """
